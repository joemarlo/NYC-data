...
ggplot() +
  geom_polygon(data = nyc.df, aes(x = long, y = lat, group = group), fill = "grey") +
...
p1 <- tidybike.df[samp_rows,] %>%
  ...
  geom_polygon(data = nyc.df,
               aes(x = long, y = lat, group = group),
               fill = "grey") +
  geom_point(aes(x = Long, y = Lat, color = Type), size = 3, alpha = 0.05) +
...
ggsave(filename = "Plots/Commuting.png",
       plot = p1,
       width = 240,
       height = 170,
       units = "mm")
